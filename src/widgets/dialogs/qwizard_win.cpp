...
{
    static const auto pDwmDefWindowProc =
        reinterpret_cast<decltype(&::DwmDefWindowProc)>(
            QApiCache::instance().get(QApiCache::SD_DWMAPI, "DwmDefWindowProc"_L1));
    switch (msg->message) {
    case WM_NCHITTEST: {
        LRESULT lResult;
        // Perform hit testing using DWM
        if (pDwmDefWindowProc && pDwmDefWindowProc(
