...
{
    static const auto pAreDpiAwarenessContextsEqual =
        reinterpret_cast<decltype(&::AreDpiAwarenessContextsEqual)>(
            QApiCache::instance().get(QApiCache::SD_User32, "AreDpiAwarenessContextsEqual"_L1));
    static const auto pIsValidDpiAwarenessContext =
        reinterpret_cast<decltype(&::IsValidDpiAwarenessContext)>(
            QApiCache::instance().get(QApiCache::SD_User32, "IsValidDpiAwarenessContext"_L1));
    if (!pIsValidDpiAwarenessContext || !pAreDpiAwarenessContextsEqual) {
        return QtWindows::DpiAwareness::Invalid;
    }
    // IsValidDpiAwarenessContext() will handle the NULL pointer case.
    if (!pIsValidDpiAwarenessContext(context))
        return QtWindows::DpiAwareness::Invalid;
    if (pAreDpiAwarenessContextsEqual(context, DPI_AWARENESS_CONTEXT_UNAWARE_GDISCALED))
        return QtWindows::DpiAwareness::Unaware_GdiScaled;
    if (pAreDpiAwarenessContextsEqual(context, DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2))
        return QtWindows::DpiAwareness::PerMonitorVersion2;
    if (pAreDpiAwarenessContextsEqual(context, DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE))
        return QtWindows::DpiAwareness::PerMonitor;
    if (pAreDpiAwarenessContextsEqual(context, DPI_AWARENESS_CONTEXT_SYSTEM_AWARE))
        return QtWindows::DpiAwareness::System;
    if (pAreDpiAwarenessContextsEqual(context, DPI_AWARENESS_CONTEXT_UNAWARE))
...
QtWindows::DpiAwareness QWindowsContext::windowDpiAwareness(HWND hwnd)
{
    if (!hwnd)
        return QtWindows::DpiAwareness::Invalid;
    static const auto pGetWindowDpiAwarenessContext =
        reinterpret_cast<decltype(&::GetWindowDpiAwarenessContext)>(
            QApiCache::instance().get(QApiCache::SD_User32, "GetWindowDpiAwarenessContext"_L1));
    if (pGetWindowDpiAwarenessContext) {
        const auto context = pGetWindowDpiAwarenessContext(hwnd);
...
QtWindows::DpiAwareness QWindowsContext::processDpiAwareness()
{
    ...
    // it as an equivalent.
    static const auto pGetThreadDpiAwarenessContext =
        reinterpret_cast<decltype(&::GetThreadDpiAwarenessContext)>(
            QApiCache::instance().get(QApiCache::SD_User32, "GetThreadDpiAwarenessContext"_L1));
    if (pGetThreadDpiAwarenessContext) {
        const auto context = pGetThreadDpiAwarenessContext();
        return dpiAwarenessContextToQtDpiAwareness(context);
    }
    static const auto pGetProcessDpiAwareness =
        reinterpret_cast<decltype(&::GetProcessDpiAwareness)>(
            QApiCache::instance().get(QApiCache::SD_SHCore, "GetProcessDpiAwareness"_L1));
    if (pGetProcessDpiAwareness) {
        PROCESS_DPI_AWARENESS pda = PROCESS_DPI_UNAWARE;
        const HRESULT hr = pGetProcessDpiAwareness(nullptr, &pda);
        if (SUCCEEDED(hr)) {
...
bool QWindowsContext::setProcessDpiAwareness(QtWindows::DpiAwareness dpiAwareness)
{
    ...
    static const auto pSetProcessDpiAwarenessContext =
        reinterpret_cast<decltype(&::SetProcessDpiAwarenessContext)>(
            QApiCache::instance().get(QApiCache::SD_User32, "SetProcessDpiAwarenessContext"_L1));
    if (pIsValidDpiAwarenessContext && pSetProcessDpiAwarenessContext) {
        ...
        if (!pSetProcessDpiAwarenessContext(context)) {
    ...
    static const auto pSetProcessDpiAwareness =
        reinterpret_cast<decltype(&::SetProcessDpiAwareness)>(
            QApiCache::instance().get(QApiCache::SD_SHCore, "SetProcessDpiAwareness"_L1));
                                           ...
                                           unsigned dpi)
{
    ...
    const BOOL result = (dpi != 0) && (pSystemParametersInfoForDpi != nullptr)
...
{
    static const auto pEnableNonClientDpiScaling =
        reinterpret_cast<decltype(&::EnableNonClientDpiScaling)>(
            QApiCache::instance().get(QApiCache::SD_User32, "EnableNonClientDpiScaling"_L1));
...
UINT QWindowsContext::getDpiForWindow(const HWND hWnd)
{
    ...
    static const auto pGetDpiForWindow =
        reinterpret_cast<decltype(&::GetDpiForWindow)>(
            QApiCache::instance().get(QApiCache::SD_User32, "GetDpiForWindow"_L1));
    if (pGetDpiForWindow)
...
UINT QWindowsContext::getDpiForMonitor(const HMONITOR hMonitor)
{
    ...
    static const auto pGetDpiForMonitor =
        reinterpret_cast<decltype(&::GetDpiForMonitor)>(
            QApiCache::instance().get(QApiCache::SD_SHCore, "GetDpiForMonitor"_L1));
    if (pGetDpiForMonitor) {
        UINT dpiX = 0;
        UINT dpiY = 0;
        const HRESULT hr = pGetDpiForMonitor(hMonitor, MDT_EFFECTIVE_DPI, &dpiX, &dpiY);
...
int QWindowsContext::getResizeBorderThicknessForDpi(const UINT dpi)
{
    ...
    static const auto pGetSystemMetricsForDpi =
        reinterpret_cast<decltype(&::GetSystemMetricsForDpi)>(
            QApiCache::instance().get(QApiCache::SD_User32, "GetSystemMetricsForDpi"_L1));
...
int QWindowsContext::getTitleBarHeightForDpi(const UINT dpi)
{
    ...
    static const auto pGetSystemMetricsForDpi =
        reinterpret_cast<decltype(&::GetSystemMetricsForDpi)>(
            QApiCache::instance().get(QApiCache::SD_User32, "GetSystemMetricsForDpi"_L1));
    if (pGetSystemMetricsForDpi) {
        return border + pGetSystemMetricsForDpi(SM_CYCAPTION, dpi);
    }
...
QWindowsApi::QWindowsApi() = default;

QWindowsApi::~QWindowsApi() = default;

bool QWindowsApi::supportsPointerApi()
{
    static const bool result =
        QApiCache::instance().get(QApiCache::SD_User32, "GetPointerType"_L1) != nullptr &&
        QApiCache::instance().get(QApiCache::SD_User32, "GetPointerInfo"_L1) != nullptr &&
        QApiCache::instance().get(QApiCache::SD_User32, "GetPointerFrameTouchInfo"_L1) != nullptr &&
        QApiCache::instance().get(QApiCache::SD_User32, "GetPointerFrameTouchInfoHistory"_L1) != nullptr &&
        QApiCache::instance().get(QApiCache::SD_User32, "GetPointerPenInfo"_L1) != nullptr &&
        QApiCache::instance().get(QApiCache::SD_User32, "GetPointerPenInfoHistory"_L1) != nullptr &&
        QApiCache::instance().get(QApiCache::SD_User32, "SkipPointerFrameMessages"_L1) != nullptr &&
        QApiCache::instance().get(QApiCache::SD_User32, "GetPointerDeviceRects"_L1) != nullptr;
    return result;
}
